...
// Updated sendWhatsAppNotification with better date handling
const sendWhatsAppNotification = async (phoneNumber, vehicle, reminder) => {
  try {
    // Validate required parameters
    if (!phoneNumber || !vehicle || !reminder) {
    ...
    const intervalMonths = serviceIntervals[reminder.serviceType] || 6;
    nextServiceDate.setMonth(nextServiceDate.getMonth() + intervalMonths);

    // Create message with fallbacks for missing data
    const message = `
🚗 *SERVICE REMINDER* 🚗

*Vehicle:* ${vehicle.make || 'N/A'} ${vehicle.model || 'N/A'}
*Reg No:* ${vehicle.registrationNumber || 'N/A'}
*Service Type:* ${reminder.serviceType || 'N/A'}
*Due Date:* ${formatDate(reminder.dueDate)}
*Priority:* ${reminder.priority || 'Medium'}

*Next Recommended Service:* ~${formatDate(nextServiceDate)}~
...
const formatPhoneNumber = (phoneNumber) => {
  ...
  // Handle Sri Lankan numbers (assume numbers starting with 0 are local)
  if (digitsOnly.startsWith('0') && digitsOnly.length === 10) {
    return `94${digitsOnly.substring(1)}@c.us`;
  }
  
  // Handle international numbers (must include country code)
...
// Updated sendServiceNotification with safe date handling
const sendServiceNotification = (email, vehicle, reminder) => {
  try {
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #ddd; border-radius: 5px;">
          ...
          <p><strong>Priority:</strong> ${reminder.priority || 'Medium'}</p>
    ...
    console.error('Error in sendServiceNotification:', err);
  }
};
  ...
  try {
    ...
    if (vehicleExists.owner) {
      ...
      if (owner) {
        ...
        if (owner.contact) {
          sendWhatsAppNotification(owner.contact, vehicleExists, newReminder);
        }
      }
    }
